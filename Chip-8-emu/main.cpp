...
bool loadSound()
{
	...
	// Load the .wav file
	if (!SDL_LoadWAV("assets/intro.wav", &spec, &audio_buf, &audio_len))
	{
...
bool loadIntro()
{
	...
	intro = IMG_LoadTexture(renderer, "assets/intro.png");
	if (intro == NULL)
	{
...
{
	// Load an image into a texture
	menu_file = IMG_LoadTexture(renderer, "assets/menu_file.png");
	if (menu_file == NULL)
	{
